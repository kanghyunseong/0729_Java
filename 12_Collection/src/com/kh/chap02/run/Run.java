...
public class Run {
	public static void main(String[] args) {
		 ...
		 *
		 * Abstract Data Type ==> Interface + 구현체 => Class + 알고리즘 => method
		 *
		 * =========================================================
		 *
		 * 배열과 컬렉션의 차이점
		 ...
		 * - 배열의 아주 쬐끔 불편한 점
		 *
		 * 1. 배열을 사용하려고 할 때 꼭 크기를 할당을 해야함 => 한 번 지정된 크기는 변경이 불가능 2. 배열 중간 위치에 새로운 요소를
		 * 추가하거나 삭제할 때 기존값을 밀어주거나 땡기는 코드를 써야함
		 *
		 * - 컬렉션 1. 크기의 제약이 없음 => 처음 컬렉션을 사용할 때 크기를 지정하지 않더라도 사용할 수 있음 => 만약 요소가 추가되면서
		 * 크기를 넘어서는 상황이 발생하면 알아서 확장됨 2. 중간에 값을 추가하거나 삭제하는 경우 기존값을 밀어주거나 땡겨주는 로직이 이미 구현되어
		 * 있음 => 그때그때 필요한 메소드들을 호출해서 사용하면 됨
		 *
		 * -- 중립 - 배열의 경우 하나의 타입의 자료형만 사용할 수 있음 - 컬렉션의 경우 여러 타입의 데이터들을 관리할 수 있음 : 같은 타입의
		 * 데이터만 묶어서도 사용가능
		 *
		 * 사용할 데이터의 개수가 명확히 정해져있고, 조회의 목적 => 배열 데이터들의 추가, 수정, 삭제가 많이 일어난다 => 컬렉션
		 *
		 * 개념적인 컬렉션의 종류
		 *
		 * - List 계열 : 다루고자 하는 값(Value)만 저장 / 저장 시 순서 유지(Index), 중복값 허용 => LinkedList,
		 * ArrayList, Vector -> ArrayList
		 *
		 * - Set 계열 : 다루고자 하는 값(Value)만 저장 / 저장 시 순서 유지 X, 중복값 허용 X => HashSet, TreeSet
		 * -> HashSet
		 *
		 * - Map 계열 : 키(Key) - 값(Value)세트로 저장 / 저장 시 순서 유지 X, 키 중복X/ 값 중복 O => HashMap,
		 * TreeMap, MultiValueMap... -> HashMap, Properties
		 */
		...
		/*
		 * arr[0] = new Plant(); arr[1] = new Plant();
		 */
		// 1. 비어있는 List에 요소를 추가
		// add(E e) : 해당 리스트 마지막에 인자로 전달된 요소를 추가해주는 메소드
		// E -> element : 제네릭
		/*
		 * Plant[] p = new Plant[3]; p[0] = new Plant("서양란", "호접란"); p[1] = new
		 * Plant("동양란", "황룡란"); p[2] = new Plant("관엽", "금전수");
		 * System.out.println(Arrays.toString(p));
		 */
		plants.add(new Plant("서양란", "호접란"));
		plants.add(new Plant("동양란", "황룡란"));
		plants.add(new Plant("관엽", "금전수"));
		plants.add("식물 끝!");
		System.out.println(plants);

		// 기존 Obejct[]의 크기를 넘어서는 요소가 추가될 경우
		// Arrays.copyOf메소드를 이용하여 깊은 복사로 더 큰 배열로 할당함
		// Obejct타입으로 요소를 관리하기 떄문에 여러 자료형의 요소를 하나의 리스트에 담을 수 있음
		...
		// index1 부터 index2까지의 요소를 추출해서 리스트를 반환
		...
		plants.addAll(sub);
		System.out.println(plants); // 리스트는 데이터 값이 중복될 수 있음
		
		// 8. 리스트에 현재 요소가 있는지 없는지 판별하는 메소드
		// isEmpty() : 리스트에 요소가 존재하지 않는다면 true /  요소가 하나라도 있다면 false;
