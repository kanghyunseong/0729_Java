...
public class Member {
	 ...
	 * 생성자의 역할
	 * -- 기술적인 관점 --
	 * => 객체가 생성될 때 어떤 상태로 시작을 해야하는가
	 * -- 개념적인 관점 --
	 * => 객체가 처음부터 유효한 상태를 가지도록 강제하기위해서
	 * 
	 * 생성자의 종류
	 * 
	 * 1. 메개변수가 없는 생성자 => 기본생성자
	 * 2. 매개변수가 존재하는 생성자
	...
	//public Member() {
		...
		// 클래스 내부에서 생성자를 하나도 작성하지 않으면 JVM이 기본생성자를 자동으로 만들어줌!
		// 기본생성자를 항상 작성하자! ★★★★★★
	//}
	
	// public member() <-- 메소드라고 인식
	
	public Member() {} //<---얘는 무조건 있어야함!
	//매개변수가 존재하는 생성자
	 ...
	 * 2. 반환형이 존재허서는 안됨! (메소드로 판단)
	 * 3. 매개변수 생성자를 명시적으로 작성하면 기본생성자를 만들어주지 않음!
	 * => 우리는 꼭 기본생성자를 억지로 열심히 작성하는 습관을 들여야함!
	...
	// [ 메소드부 ]
	// 16:00s는 여러분들이 힘든 시긴이 때문에, 실습을 드리겠습니다.
	// 모든 필드에 대한 getter() / setter() 만들기 
	// 모든 필드의 정보를 합쳐서 반환하는 info()만들기
	 ...
	 * 3) 모든 필드의 정보를 합쳐서 문자열의 형태로 반환해주는 info생성
	 * 4) 기본 생성자 및 모든 필드를 매개변수로 가지고 있는 생성자 생성
	 * 5) 직접접근으로 사용하고 있던 실행클래스의 코드들을 전부 다 getter / setter로 대체
	 * 
	 */
}
