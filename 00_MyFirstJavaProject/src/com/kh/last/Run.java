...
public class Run {
	...
	public static void main(String[] args) {
		...
		System.out.println("===============================");
		names.stream().map(name -> name + "(은)는 정말 못말려 ~~~").forEach(System.out::println);
		...
		// map(), filter() <- 많이 씀
		// filter(필터링, 조건에 맞는것만 필터링 할 수 있음)

		List<String> coffee = Arrays.asList("아메리카노", "라뗴", "콜드브루", "에스프레소", "헤이즐넛");
		...
		List<Integer> lengths = coffee.stream().map(c -> c.length()).collect(Collectors.toList());
		System.out.println(lengths);
		coffee.stream().map(c -> c + "의 길이: " + c.length())
				// .forEach(1 -> System.out.prinln(1));
