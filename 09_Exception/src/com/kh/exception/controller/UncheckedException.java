...
public class UncheckedException {
	 ...
	 * 
	 * - ArrayIndexOutOfBoundsException => 배열의 부적절한 인덱스로 접근했을 떄 발생하는 예외
	 * 
	 * - ClassCastException => 허용할 수 없는 형변환을 진행할 경우 발생하는 예외
	 * 
	 * -ArithmeticException => 나누기 연산을 0으로 나누면 발생하는 예외
	 * 
	 * - NegativeArraySizeException => 배열을 할당할 떄 크기를 음수로 지정하면 발생하는 예외
	 * 
	...
	public void method1() {
		...
		/*
		 * // 방법 1 : 예외발생 자체를 원천 차단해버림 if(num2 != 0) { System.out.println("연산 결과 : " +
		 * (num1 / num2)); } else { System.out.println("너 혼꾸녕 나고싶어?"); }
		 */
		// 방법 2 : 예외처리를 진행
		// 예외처리 :
		// 예외 상황을 감지하고 예외 상황이 발생 시
		 ...
		 * [ 표현법 ] try {
		 * 
		 * (num1 / num2) // 예외가 발생할수도 있는 구문 넣기 } catch(발생할 예외 클래스명 변수명) { => 매개변수
		 * 
		 * 해당 예외가 발생했을 떄 실행할 구문(코드) }
		 * 
		 * 
		 */
		...
		try {
			...
			System.out.println("두 번쨰 정수에 0을 입력하시면 나눌 수가 없습니다.");
	...
	public void homeWork() {
		...
		while (true) {
			...
			try {
				...
				// 예외처리 시 catch문에 적야아하는 내용이 출력문은 아님
			}
			sc.next();
	...
	public void method3() {
		...
		try {
			...
			System.out.println("100번쨰 인덱스 값 : " + arr[100]);
			
		} catch(RuntimeException e) {
			System.out.println("아마도 음수를 입력했거나.. 100보다 크지않아서");
		} 
		...
		/*catch (InputMismatchException e) {
			...
			System.out.println("101보다 큰 수!");
		}*/
		
//		try {
//			System.out.println("100번쨰 인덱스 값 : " + arr[100]);
//		} catch (ArrayIndexOutOfBoundsException e) {
//			System.out.println("값이없노");
//		}
//		
