...
import com.kh.plant.model.dao.*;
import com.kh.plant.model.vo.*;

// Controller : 사용자가 View를 통해 요청했을 때
// 요청을 받아 처리해준 뒤 다시 처리결과를 View로 반환해줌
public class PlantController {
	// 식물저장소
	private List<Plant> plants = new ArrayList(10);
	...
	public List<Plant> readAll() {
		// Plant타입의 객체들의 주소가 요소로 존재하는 Object[]을 Videw에게 반환
	...
	public List<Plant> findByKeyWord(String keyword) {
		...
		for(int i = 0; i < plants.size(); i++) {
			...
			if(plant.getName().contains(keyword)||
			   plant.getName().contains(keyword)) {
				searched.add(plant);
			}
		}
		return searched;
