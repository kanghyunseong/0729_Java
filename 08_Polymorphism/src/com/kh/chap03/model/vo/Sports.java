 ...
 * 
 * => 기술적으로는 개발자가 판단했을 떄 이 클래스는 부모클래스 역할로만 사용할 것이고,
 * 	  객세 생성은 불가능해야한다 라고 생각이 들면 추상클래소 선언
 * 
 * => 개념적으로는 개발자가 판단했을 떄 해당 클래스가 구체적으로 구현된 상태가 아닌 것 같다 하는
 * 생각이 들면 추상클래스로 선언
 * 
 * => 객체 생성은 불가능하게 하고 공통적인 속성 및 타입만 다형성을 적용하게 하고싶다.
 */
